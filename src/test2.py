# ============================================================
# test2.py — KaUDC004A 变频模块控制类用法演示
#
# 依次调用 KaUDC004A 类的各个功能并打印结果，涵盖：
#   系统类(版本/温度) → DAC校准 → 衰减/增益 → 本振切换 → Flash回读
#
# 用法要点：
#   - DAC(0xF1) 写校准码时会把上变频本振切到 DACch 对应频段，
#     因此 DAC 设置放在本振设置之前，最后由 set_lo 确定测试频段
#   - 0x12 本振固化约需 1 秒，设置后 sleep 等待固化完成
#   - 校准码存于模块 Flash，正式测试只需 set_lo 切频段即自动生效
#
# 运行前：
